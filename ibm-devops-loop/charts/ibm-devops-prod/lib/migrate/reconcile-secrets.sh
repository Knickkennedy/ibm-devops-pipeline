...
check-pre-reqs() {
  ...
  for u in ${CMD_KUBECTL:-kubectl} curl jq
  do
    if command -v "$u" >/dev/null; then
      echo "  $u found in PATH"
    else
      >&2 echo "ERROR: \"$u\" not found in PATH. Please install it."
      if command -v apt-get >/dev/null; then
        >&2 echo "       sudo apt-get install $u"
      else
        >&2 echo "       sudo dnf install $u"
      fi
      exit 1
    fi
  done
...
keycloak-get-password() {
  secret-get -s "$release-keycloak" -k password
}
...
keycloak-get-token() {
  ...
  local token retries
  retries=3
  while [[ -z "$token" ]] && [ $retries -gt 0 ]; do
    local tokens="$($curl -s -X POST "$keycloak_url/realms/master/protocol/openid-connect/token" \
    ...
    token=$(sed -r -n 's/.*"access_token":"([^"]*)".*/\1/p' <<< "$tokens")
    retries=$(( retries - 1 ))
    if [[ -z "$token" ]] && [ $retries -gt 0 ]; then
      >&2 echo "WARNING: Keycloak returned no access token, retrying in 5 seconds..."
      sleep 5
    fi
  done
...
keycloak-get-user() {
  ${CMD_KUBECTL:-kubectl} get statefulset "$release-ssocloak" -n "$namespace" \
...
pod-status-ready() {
  ${CMD_KUBECTL:-kubectl} get pod -ojsonpath='{.status.containerStatuses[0].ready}' "$@"
}
...
postgres-set-password() {
  ...
  local -r postgres_pod="${pod:-$release-postgresql-0}"
  local -r postgres_password="$(secret-get -s "$postgres_secret" -k postgresql-password)"

  ${CMD_KUBECTL:-kubectl} exec -n "$namespace" "$postgres_pod" -- bash -c \
...
rabbitmq-get-password() {
  secret-get -s "$release-rabbitmq" -k rabbitmq-password
}
...
rabbitmq-set-password() {
  ...
  ${CMD_KUBECTL:-kubectl} exec -n "$namespace" "$release-rabbitmq-0" -- rabbitmqctl change_password "$username" "$password"
...
secret-get() {
  ...
  ${CMD_KUBECTL:-kubectl} get secret -n "$namespace" "$secret" -o jsonpath="{.data.$key}" | base64 --decode
}
...
postgres-set-password -u keycloak -s "$release-keycloak" -e "$(keycloak-script-delete-user -u "$keycloak_user")"

echo "Restarting Keycloak to create new \"$keycloak_user\" user"
${CMD_KUBECTL:-kubectl} delete pod "$keycloak_pod" -n "$namespace"
echo

postgres-set-password -u datasets
postgres-set-password -u execution
postgres-set-password -u gateway -s "$release-core"
postgres-set-password -u results
postgres-set-password -u rm
postgres-set-password -u testassets -s "$release-tam"
postgres-set-password -u postgres -s "$release-postgresql"
postgres-set-password -u extensions
...
keycloak_clientid=testserver
: "${REALM_NAME:=testserver}"
keycloak_url=$(${CMD_KUBECTL:-kubectl} exec -n "$namespace" "$keycloak_pod" -- sh -c 'echo https://$KC_HOSTNAME/auth')
keycloak_token="$(keycloak-get-token -u "$keycloak_user" -p "$(keycloak-get-password)")"

echo "Setting Keycloak client secret for \"$keycloak_clientid\" (used by gateway) at \"$keycloak_url/admin/realms/$REALM_NAME\""

client="$($curl -s "$keycloak_url/admin/realms/$REALM_NAME/clients?clientId=$keycloak_clientid" \
  -H "Authorization: Bearer $keycloak_token" \
  | jq ".[0] + {secret:\"$(secret-get -s "$release-gateway" -k oauth-client-secret)\"}")"

$curl -sw '%{http_code}' -X PUT "$keycloak_url/admin/realms/$REALM_NAME/clients/$(jq -r .id <<< "$client")" \
  -H 'Content-Type: application/json' \
  -H "Authorization: Bearer $keycloak_token" --data-binary @- <<< "$client"; echo

server_host=$(${CMD_KUBECTL:-kubectl} exec -n "$namespace" "$keycloak_pod" -- sh -c 'echo $INGRESS_DOMAIN')
echo
echo "Password reconciliation completed successfully, wait for pods to become ready:"
echo "  ${CMD_KUBECTL:-kubectl} get pods -n $namespace"
echo
echo "Next Steps"
...
echo
echo "To perform this action an offline token belonging to a user with the Administrator role is required along with the PasswordAutoGenSeed used in the previous install. These values should be used in place of the examples shown below for OFFLINE_TOKEN and OLD_HELM_SEED before running the given curl command."
echo
...
echo "${CMD_KUBECTL:-kubectl} logs -n $namespace -lapp.kubernetes.io/name=gateway -f"
