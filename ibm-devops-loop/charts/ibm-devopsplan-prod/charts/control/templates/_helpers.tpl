...
{{- if (and ( or .Values.global.platform.enabled .Values.global.plan.enabled ) .Values.global.persistence (eq $storageClass "")) }}
{{- $storageClass2 := (default "" .Values.global.persistence.rwoStorageClass) }}
{{- if $storageClass2 }}
storageClassName: {{ $storageClass2 | quote }}
{{- end }}
{{- else }}
{{- if $storageClass }}
